...
## ✅ Completed Tasks
- **Ground-Up Redesign — "Carte IGN" Territory Sheet** (branch `nicnac/feat/ground-up-redesign`, completed & running locally):
  - Replaced the "Editorial Peloton" look with a topographic-sheet world: the site is printed as a map of the club's real territory around the Place de la Féchère (5 m contours, rivers, roads, rail, woods), generated by `scripts/generate-territory-map.mjs` from AWS Terrain Tiles and OpenStreetMap into `public/carte/*.svg` (CSS mask layers) and `app/data/territory.json`.
  - Rewrote the design system in `app/globals.css` (paper/ink/brand/night tokens, map spot inks for day and night sheets, neat-line and scale-strip utilities, reduced-motion rules); Archivo variable font (`wdth` axis) replaces Poppins; updated `DESIGN.md` to formally document "La Feuille de Blanmont".
  - New home (`app/page.tsx`, `app/components/home/*`): full-bleed territory cover with canvas wind field, departure cartouche, weekend poll board, pace groups as road classes, season timetable, Carré Vert band, photo plates and newcomer join route.
  - Restored the club photo slider on the new home as its own band under the cover (`app/components/home/HomeSlider.tsx`, slides managed in `/admin/hero`); `HeroTelemetryFrame` gained a `showCards` prop so the band shows photos only (the info cards stay in the cover margin), and its frame/controls were flattened (no shadow, no backdrop blur, no pill buttons) to match the sheet. Covered by `tests/components/HeroTelemetryFrame.test.tsx` and `tests/components/HomeSlider.test.tsx`.
  - New map component library `app/components/carte/*` (`TerritoryMap`, `SheetHeader`, `WindField`, `WindRose`, `WeatherGlyph`, `RoadSwatch`, `ScaleBar`, `GeodeticMark`).
  - Unified all pages across the platform onto `SheetHeader`: calendrier, sondage, le-club, equipement, rejoindre, sécurité, membres, blog, galerie, classement, login, 404, checkout, traces (`TraceList`), trace detail (`/traces/[id]`), trace edit, and Saturday ride.
  - Added modern `SheetHeaderSkeleton` loading states to `/traces/loading.tsx`.
  - Added unit test suites for `SheetHeader` and carte components (`tests/components/SheetHeader.test.tsx`, `tests/components/carteComponents.test.tsx`).
  - *Status*: Verified with full unit test suite (88 files, 629 tests passing), `tsc --noEmit` (0 errors), Next.js production build (104 routes generated), and local development server running at `http://localhost:3000`.
