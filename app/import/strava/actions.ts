...
import { createTrace, getTracesSchema, deleteTrace } from '../../lib/firebase';
import {
...
export async function importStravaTraceAction(
  activity: any,
  ...
  try {
    ...
    photoUrls = photos
      .map((p) => p.urls['2048'] || p.urls['1024'] || p.urls['600'])
      .filter(Boolean);
  } catch (e) {
  ...
  const result = await createTrace({
    ...
    description:
      validatedActivity.description ||
      `Imported from Strava. Distance: ${(validatedActivity.distance / 1000).toFixed(1)}km`,
