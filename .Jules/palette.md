...
## 2024-05-18 - Added aria-pressed and focus-visible to custom toggle buttons

**Learning:** Found several custom toggle buttons (like those used for poll options) lacking `aria-pressed` attributes and proper keyboard focus rings.
**Action:** When implementing custom interactive elements that act as toggles, always add `aria-pressed` bound to the selected state and ensure `focus-visible` classes are present for keyboard accessibility.
